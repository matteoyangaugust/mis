...
<form class="row selectComponentTemplate swalLabelText">
            ...
            <c:forEach var="component" items="${components}">
                <c:choose>
                    <c:when test="${component.active eq 0}">
                        <input type="button" class="text-left btn btn-outline-danger" value="${component.identifier}-${component.name}(Disabled)">
                    </c:when>
                    <c:otherwise>
                        <input type="button" class="text-left btn btn-outline-success materialBtn" value="${component.identifier}-${component.name}">
                    </c:otherwise>
                </c:choose>
